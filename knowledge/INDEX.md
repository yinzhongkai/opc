...
一个知识条目对应 `knowledge/<id>.md`，必需文件头为：
...
共享岗位以 `knowledge` 定义基础要求。项目成员表以 `roleKnowledge` 为相应岗位补充知识。初始化按[协议](../SESSION_PROTOCOL.md)将二者合并去重，并完整读取对应正文。
...
修改或退役正文前，检查共享岗位的基础引用和全部项目的补充引用，再确定受影响的成员。退役前迁移仍在使用的引用并保留历史文件。文档更新不代表其他会话已经读取；成员开始新任务时按初始化协议刷新当前可访问的资料。
