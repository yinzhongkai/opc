...
- 当前阶段：一期 MVP 集成收口、产品效果评估与个人未签名 Windows 交付验证；T-029 因真实 VFR、真人评审和确认的 Windows 基准运行条件缺失处于 blocked，T-037 正在按 D-012 收口个人未签名工程包，T-038 尚待前置完成。
...
- 交付边界：按 [D-012](DECISIONS.md)，一期 Windows 交付仅限用户本人在自有电脑上使用的 `unsigned-engineering` 个人工程包，不公开分发、不交付第三方，也不承诺 SAC/WDAC 兼容；验证转到允许未签名 Win32 程序运行的自有干净 Windows x64 电脑或虚拟机，且不以关闭主机安全策略作为项目方案。
- 非目标：一期不生成完整歌曲，不纳入 VST3、Blender/GPLA、专业合成器、复杂效果器、现场演出集成、Linux 适配或默认深度学习模型。
- 交付物与验收要求：当前成果均以 `artifacts/` 索引和任务证据为准；产品效果须完成 T-029，端到端故障恢复及 G0～G4 证据须完成 T-022，个人未签名 Windows 交付须完成 T-036～T-038。任务完成不自动代表成果批准、公开分发或生产发布批准。
- 时间、资源与其他约束：尚未确认工期、预算、真实 VFR 原始素材、真人评审资源、最低 Windows 版本、正式导出格式、产品默认音色和视觉风格；公共签名证书、签名主体、发布渠道及独立 GUI 安装器已由 D-012 移出本阶段必选门禁，未来扩大分发范围时须重新决定。用户保证当前仓库各成员会话串行执行。
