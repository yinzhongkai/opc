...
周二早上，阿凯工位。白板上的启动链图还挂着，BL31 那一格旁边是老周昨天下班前写的一行字：启动链，下半段已通电。再往下，BL33 那格还是空的。

老周端着杯子过来，指着那个空格："昨天日志停在哪？"

"等 BL33。"阿凯答。

"今天把它填上。"老周说，"U-Boot 这个东西规矩多，defconfig、dts、env、boot script，每一样都要管。"
...
"不动。"老周说，"TF-A 一行都不动。fip 那条路 chapter 10 打通启动链的时候才走。"
...
"填进去谁加载它？"老周反问，"BL2 从哪读 fip、BL31 怎么把 BL33 从包里请出来、往哪跳——这是启动链两级之间怎么握手的事，不是'文件存在'的事。今天 U-Boot 用 `-bios` 单跑就够了，握手留给 chapter 10。"

不过老周让阿凯看了一眼 meta-arm 配方里备好的机关——这个旋钮 chapter 10 会用到，今天先混个脸熟：
...
#### 7.2.1 老周同一问法，答案自己冒出来

"配方从哪来。"老周把昨天的问题原样抛回来，"自己写一份，还是……？"
...
第一眼看第二行：`PROVIDES = "virtual/bootloader"`。chapter 6 老周说过"`virtual/bootloader` 有 OE-Core 里现成的 PROVIDES，chapter 7 你会用到真的"——就是这一行，虚包真的存在。第三段那个 `${@bb.utils.contains(...)}` 是个条件依赖开关：`UBOOT_ENV_SUFFIX` 等于 `scr` 时才把 `u-boot-mkimage-native` 拉进 DEPENDS——7.6 讲 boot script 时这条管道的主角。`inherit uboot-config` 引出的 class 是 7.3 的主角。
...
老周点点头，这次他给了正面回答："名字没撒谎，名字说的是'这份配方文件生在 2024.01 时代'。是我们把源码换成 2024.04 之后，名字才开始名不副实。"
...
"记住这个错位不是事故，是常态。"老周补了一句，"发行版的配方文件版本和它实际能构建的源码版本，本来就靠 bbappend 这层隔开。你要做的不是消灭错位，是让错位在纸面上说得清。"
...
"那如果集成的时候想临时拧一个配置项呢？"老周问，"比如调试期想开个 CONFIG，又不想动 fork 仓库。"
...
这个问题问到了点子上，老周却没收："patch 头部有答案，自己去翻。"
...
"预判错不错不重要，**动作不能省**。"老周说，"换大版本后旧 patch 清单逐条重审，这是正常工序，不是出了事才补的救。跳过它的两种结局我画给你看——运气好，do_patch 安静通过，可某条其实上游已经修了，你背着一条空补丁而不自知；运气差，do_patch 直接报 context mismatch 把你拦下。**do_patch 失败是信号，不是意外**——它在告诉你清单和源码版本对不上了。"
...
"是两份。"老周说，"今天先把这条界限划了，免得 chapter 8 打架。"
...
"`u-boot.inc` 里留着可选通道——`UBOOT_DTB`、`UBOOT_DTB_BINARY` 设了，do_deploy 会把编出的 dtb 一并拷进 deploy 目录。"老周说，"tiger 要不要用这条通道，等 u-boot-tiger 的 defconfig 定了再定。今天记住一件事：**跟着 u-boot.bin 走的那份设备树是 U-Boot 自己带的，不来自内核仓库**——将来在真板上排查'U-Boot 认得出、内核认不出'这类问题，先想清楚自己在看哪一份。"
...
老周这次答了："算契约。chapter 8、chapter 9 照这个契约交付文件和卷名，chapter 10 整条链才敢通电。骨架先立在纸面上，后面两章是照着它填空，不是到时候再临时商量。"
...
"两章互为镜像。"老周说，"chapter 6 你查出来'虚包不存在、写了没人听'，今天查出来'虚包存在、不写没人替你仲裁'；chapter 6 我那句前瞻估错了虚包，你这句前瞻估错了名字。同一套查证动作，两种结论——结论可以变，动作不能省。"
...
"别急着收。"老周说，"反着试一次——照文件名写 `"2024.01%"`，看着也名正言顺，会发生什么。"
...
"直觉这次对，是碰巧踩在机制上。"老周说，"不反着试一次，不知道踩的是机制还是运气。落盘的是 `2024.04%`，带走的是那次反试。"
...
"会跑，还不会接力。"老周说，"它现在拿不到内核——内核还不存在；拿到了也不知道从哪读——NAND 还没建。但 BL33 这格从今天起是实的了。"
...
这次 do_fetch 重新拉取、下游任务连锁重跑，QEMU 里 `printenv` 出现了新的 bootcmd。老周听完复盘，只补了一句："直拉仓库流的纪律就一条——**仓库动一次，SRCREV 跟一次**。写进提交信息里也好，写进流程里也好，总之别指望有人提醒你。"
...
"死得痛快的错都是好错。"老周说，"怕的是 7.9.1 那种一声不吭的。"
...
老周下班前走到白板跟前，把 BL33 那格涂实了，在原来的那行字下面添了一句：BL33 报到，下半链齐活——等内核。
