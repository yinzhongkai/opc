...
- 修订范围：用户两次明确重开 T-026 后修复 `T026-DEFECT-001`～`006`；本轮增量聚焦 `005`、`006`
- 成果：A-027 0.3；UI bridge 0.3.0/schema 3；真实 UI/worker 双进程；pending 手势事务；事件音轨试听/导出；操作级取消；异步项目 IO 与有界分块混音
...
2. 既有黄金媒体直跑 `space_rhythm_ui_integration_tests`：Debug 与 CI/RelWithDebInfo 均为 Qt Test 11 passed、0 failed，分别约 12.5 s 和 12.3 s。九个测试函数覆盖结果 codec/引用校验、真实 worker 导入、取消终态、取消→重连→试听、实际进程 kill 断线恢复、drag/seek pending 事务、完整六阶段路径、异步保存重开，以及大项目异步打开/失败重试。
...
6. 本轮另跑 ProjectStore 四项、音频取消、导出取消、IPC 取消和 JobCoordinator 取消选择集，连同其 fixture 共 9/9 通过；Debug 与 CI/RelWithDebInfo 的 UI/headless 选择集均为 5/5。
7. `git diff --check` 通过；源码扫描确认应用 QML 不包含 `Animation`/`Timer` 权威时钟，不暴露 raw `TimeNs`、revision 或 frame index。T-022 未启动、未修改。
...
| `T026-DEFECT-005` | `active_job_cancellation_` 与 `preview_cancellation_` 分域；试听另带单调操作序号，停止、重建或新试听使旧结果失效。worker 作业取消令牌不再被试听复制。 | `cancelledWorkerJobDoesNotPoisonPreviewAfterReconnect` 在真实 worker 延迟分析中取消，等待终态并重连，然后加入事件、显式启用开发音色并验证试听进入 `playing`、无错误；Debug/CI 均通过。 |
| `T026-DEFECT-006` | `open_project()` 只在 GUI 发布 `loading`/应用结果；`ProjectStore::load()`、可写性检查及测试慢 IO 在后台执行，并以 workspace generation 拒绝迟到结果。打开失败路径可由 `retryFailedOperation()` 重发同一异步命令。 | `largeProjectOpenStaysResponsiveAndFailedOpenCanRetry` 使用约 8 MiB 有效项目和 600 ms 确定性 IO 延迟，断言延迟期间 25 ms GUI heartbeat 已执行且页面仍为 `loading`；随后验证损坏文件进入结构化失败，替换为有效文件后重试恢复 `idle`。Debug/CI 均通过。 |
...
仓库中既有、hash 已登记的 `cfr_av.mkv` 可由链接的 FFmpeg 库真实探测/解码，修订后的 Debug 与 CI/RelWithDebInfo 测试程序直接运行均为 11/11；两种配置的 UI/headless 选择集也均为 5/5。CI 选择集首轮 `worker.process_smoke` 曾以退出 0、空 marker 失败，未启用 WDAC fallback；同一二进制独立复核后，单项和完整选择集复跑均通过。音频黄金生成器不依赖受阻的媒体 CLI，本轮相关选择集成功运行。由于固定 FFmpeg CLI 仍无法重新生成媒体黄金素材，包含该 fixture 的完整标准 CTest 门禁未宣称通过。
...
| 自测、任务及交接更新 | 本摘要、A-027 0.3、T-026 和 H-006 修订记录 |
