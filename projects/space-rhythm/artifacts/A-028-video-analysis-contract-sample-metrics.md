...
- 批准依据：尚无。
- 版本记录：2026-09-14，0.1，首次冻结输入/输出、三类候选、真实 `timeNs`、置信度/版本/低质量原因、合成样本配方及指标口径。2026-09-14，0.2，登记 T-028 已按冻结配方生成真实媒体和证据；契约语义、配方 hash、标注 hash 与未确认门槛均未改变。
...
T-027 冻结时不生成媒体字节，当时 `mediaStatus=specified_not_generated` 且
`actualMediaSha256=null` 是有意状态。T-028 已按原配方生成实际媒体并把当前 manifest 更新为
`mediaStatus=generated` 和真实 SHA-256；配方/标注 hash 未改变，详见 A-029 0.1。

T-028 已用项目固定 FFmpeg 基线实现生成器，生成实际字节，登记媒体 SHA-256 和探测
...
T-028 已完成并保留经典基线、负例与可复核测量；T-029 仍须单独接收和启动，本轮未执行。
