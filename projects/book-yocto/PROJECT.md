...
- 范围与非目标：范围包含技术内容的完整实测验证——实际创建四个开发态仓库并全链构建，回填全部 C-W/V 待验证项（[D-001](DECISIONS.md)，2026-09-18 确认候选 A）；术语与体例统一纳入范围（[D-005](DECISIONS.md)）。非目标：PDF/EPUB 等排版导出与出版渠道对接（[D-002](DECISIONS.md)）。
- 交付物与验收要求：交付物为 `workspace/yocto/` 的 Markdown 终稿（[D-002](DECISIONS.md)，2026-09-18）；验收标准为书稿逐章评审通过、验证按 D-001 执行完毕、学习维度以建项用户通读校验代替外部试读（[D-003](DECISIONS.md)，2026-09-18）。
