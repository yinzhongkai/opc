...
- 未完成事项：T-024/T-025 已完成；T-026 的 `T026-DEFECT-001`～`004` 已修复并通过发起人复核，但新增发现 `T026-DEFECT-005`（取消令牌泄漏到重连后的事件试听）和 `T026-DEFECT-006`（打开项目仍在 GUI 线程同步读盘），须由 ui-engineer-qt-quick-01 在用户重新授权后重开 T-026 修复并重新提交，因此 H-006 暂不关闭。视觉风格、产品文案、产品默认音色、发布导出格式、产品默认视觉模板参数、设备矩阵及可访问性/性能门槛仍需产品或对应责任人确认；A-018 CC0 映射仅能显式作为开发用途，当前 T-019 NUT/raw RGBA/PCM 仅为 `testOnly` 开发格式。
- 状态：accepted
- 创建日期：2026-09-09
- 接收反馈：ui-engineer-qt-quick-01 于 2026-09-09 完成成员会话初始化并确认接收；2026-09-11 用户明确要求继续完成 T-024，并要求 T-025、T-035 不得提前启动；T-024 完成后，用户明确确认 T-025 前置全部满足并授权执行 T-025，同时要求不提前执行 T-026。2026-09-12，用户明确要求执行 T-026，并要求接入现有真实核心、媒体、音频、渲染、T-019 时钟及冻结/安全导出能力；architect-01 登记四项复核缺陷后，用户再次明确重开 T-026，要求修复、补针对性测试、更新 A-027/T-026/H-006 后重新提交，并明确修复前不启动 T-022。
- 处理结果与证据：2026-09-11，T-024 已完成并形成 [A-023 0.1](artifacts/A-023-qt-quick-ui-information-architecture-and-design-system.md)，覆盖完整信息架构、线框、状态矩阵、交互规则、可访问性、设计系统和 ViewModel/mock 清单。T-025 随后完成并形成 [A-024 0.1](artifacts/A-024-qt-quick-workspace-and-viewmodel-bridge.md)及 [验证摘要](evidence/T-025/verification-summary.md)，交付应用壳层、拆分组件、版本化桥接、mock、SceneGraph 挂载和 headless 测试。2026-09-12，T-026 首版形成 A-027 0.1 后被发起人要求修订；用户明确重开后已形成 [A-027 0.2](artifacts/A-027-qt-quick-real-workflow-integration.md)及更新后的 [验证摘要](evidence/T-026/verification-summary.md)：bridge 0.3.0/schema 3 完成 drag/seek pending/revision 事务、真实 `LocalIpcClient` + 外部 worker 媒体/DSP、显式 A-018 CC0 事件音轨、后台主保存/autosave 及 4096-frame 分块安全导出。Debug 与 CI/RelWithDebInfo 构建、既有黄金媒体直跑 9/9 和 UI/headless 5/5 均通过，音频/播放/导出/离屏 22/22、IPC/Job/worker 12/12 通过；固定 FFmpeg CLI fixture 的 WDAC/SAC 限制继续如实保留。T-022 未启动；H-006 保持 `accepted` 等待发起人复核修订提交。
- 关闭或取消依据：尚未关闭。2026-09-12，发起人 architect-01 核对提交 `f792e64`、A-027 0.1 和 T-026 证据；标准 `ctest -L t026` 中 3 项通过、黄金素材生成仍因已登记的 FFmpeg CLI/WDAC 环境限制失败并使真实工作流未运行，改用仓库既有黄金素材、排除 fixture 自动生成后，UI bridge、真实工作流、普通/1.5× QML 和应用 smoke 共 5/5 通过。但代码审查结论为 `revise`：`T026-DEFECT-001`，事件拖动在每次 pointer move 直接提交 `MoveEvent`，结束仅清状态，缺少 A-023 要求的 ghost、单次松开提交、Escape 取消和基准修订失效处理；seek 同样缺少 pending/ack/取消语义。`T026-DEFECT-002`，导入/分析仍在 UI 进程 `std::jthread` 执行，断线仅由测试命令模拟，未消费 T-016 `LocalIpcClient`/worker 进程，不能作为 D-003 双进程链路或 T-022 的真实前置。`T026-DEFECT-003`，试听播放解码源 PCM；导出以空音色和空 mapping 调用 T-032 mixer，事件全部未映射并产生静音 PCM，不满足视频到节奏音轨的闭环。`T026-DEFECT-004`，项目保存/自动保存和导出末尾整段音频混音仍在 GUI 线程同步执行，违反 `WorkspaceService::post()` 不等待磁盘/渲染工作的契约，也使长素材取消响应和内存有界性缺少保证。用户明确重开后，负责人已按 A-027 0.2 和本轮 T-026 证据逐项修复并重新提交；architect-01 对提交 `8bdfdbb` 独立复核时，确认 drag/seek 事务、外部 worker IPC、事件音轨及后台保存/分块导出实现符合修订要求，Debug UI/headless 6/6、IPC/Job/worker 12/12，以及音频/T-019/离屏相关测试在按 GPU 环境分别执行后全部通过。继续检查恢复路径发现两项新的阻断缺陷：`T026-DEFECT-005`，`cancel_job()` 取消共享 `cancellation_` 后，`finish_job_cancelled()` 和 worker 重连都不重置它，随后 `prime_event_preview()` 复制该 cancelled token，导致试听立即失败；`T026-DEFECT-006`，`open_project()` 通过 `QTimer::singleShot(0)` 排队后仍在 GUI 回调中同步调用 `ProjectStore::load()`，排队不等于异步 IO，较大项目会阻塞界面且尚无对应恢复测试。因此本轮复核仍为 `revise`，H-006 不关闭，T-022 不启动；既有产品待确认项未作为静默默认值处理。
