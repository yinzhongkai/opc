...
- 状态：completed
- 授权来源与日期：本会话用户于 2026-09-14 明确要求刷新项目资料、核对并按条件关闭 H-001，随后登记并执行下一编号任务，形成 T-029 所需的代表视频集要求、人工标注协议、“卡点自然” rubric、评分尺度、播放条件及效果阈值，并将无法替用户决定的素材、基准硬件和阈值列为最小确认清单；明确禁止使用合成 golden 替代产品数据。
- 目标与范围：在 D-001 已确认产品范围内，把 T-029/A-030 的产品侧缺口整理成可执行输入协议；固定产品视频集的覆盖和元数据要求、人工标注与复核流程、自然度 rubric、五级评分尺度、盲评播放条件、效果与性能指标/比较符/聚合口径，并显式保留真实素材、基准 Windows 硬件和数值阈值的用户确认边界。本任务不选择具体用户视频、不替用户承诺硬件、不编造通过数值、不运行 T-029，也不引入模型。
- 输入与依赖：D-001～D-008 confirmed；A-002 0.2 approved；A-011 0.1、A-016 0.1、A-028 0.2、A-029 0.1、A-030 0.1；T-029 当前 `blocked`；H-001 的项目经理处理结果；本次用户直接授权。
- 优先级：未设定；当前用于解除 T-029 的产品输入阻塞。
- 完成条件与确认方式：形成版本化产品评估输入，覆盖代表视频切片、数据分区和许可/hash、人工标注/复核/分歧处理、人工修正计量、自然度维度与评分锚点、盲评播放条件、逐指标门槛结构和最小用户确认清单；自查与 A-030 输入字段一致，不将 T-028 合成 golden、Wine 性能或未确认数值写为产品通过基线。本任务只要求形成输入协议，不要求用户在同一任务内提供实际素材或数值。
- 进展：已形成并批准 A-031 0.2，固定 `productEvaluationInputVersion=0.1.0`。用户逐项确认 D-009：product-manager-01 依授权筛选四类 40 个 B 站真实来源并固定时间窗、`4/16/20` 分区与 slice 配额；记录当前 Windows 基准机、720p 代理、线程和预热条件；确认全部效果、人工修正、自然度、评审和性能 gate。T-028 合成 golden 和 Wine 测量仍禁止代替产品数据与 Windows 基准。
- 成果与验证证据：[A-031 0.2：T-029 视频产品评估输入](artifacts/A-031-t029-video-product-evaluation-input.md)，状态 approved；D-009 confirmed；H-012 closed。本轮只进行产品输入确认和只读素材来源/本机信息收集，没有运行 T-029。
...
- 输入与依赖：T-022、T-037；D-004～D-008、D-012、D-013；最低 Windows 版本仍待确认；A-011 0.1；H-016 closed。
- 优先级：未设定（架构建议：个人交付验证阶段必需）。
- 完成条件与确认方式：在当前 `TIGER` 上复核哈希、明确记录 unsigned 与 SAC-off 状态、依赖、安装/修复或升级/卸载/回滚、首次启动和核心工作流；记录系统版本、安全策略适用边界、环境、命令、结果、失败与未覆盖项，输出个人未签名交付证据包。不得把结果外推为 SAC/WDAC 兼容、干净 Windows 兼容矩阵、公开分发或生产发布；负责人自查。
- 进展：2026-09-15，用户先通过 D-012 将任务从签名/发布候选验证收口为个人未签名交付验证，随后通过 D-013 指定当前 `TIGER` 并手动关闭 SAC；只读状态为 `VerifiedAndReputablePolicyState=0`。任务尚未由负责人启动，历史签名状态改为如实记录 unsigned 而非通过门禁。
- 成果与验证证据：[A-011 0.1](artifacts/A-011-complete-mvp-engineering-staffing-and-task-plan.md)；实际证据暂无。
- 阻塞与下一位行动人：等待 T-022/T-037；H-016 已关闭，不再等待替代电脑、虚拟机、H-015 或签名证书。前置完成后由 release-engineer-windows-01 在当前 `TIGER` 执行。
- 更新日期：2026-09-15。
...
- 阻塞与下一位行动人：T-036 计划完成条件已满足。依据 D-012/D-013，T-037/T-038 改走当前 `TIGER` 上的个人未签名工程包路线，不再等待 H-015、H-016、签名证书、发布渠道或独立 GUI 安装器；下一步由 release-engineer-windows-01 在当前主机收口 T-037。最低 Windows 和正式格式/H.264 后端仍按各自责任链确认。
- 更新日期：2026-09-15。
...
- 状态：blocked
- 授权来源与日期：本会话用户于 2026-09-09 要求补充系统架构师判断的新增成员及成员任务并提交；同一用户于 2026-09-14 明确授权启动 T-029，并要求纳入代表产品视频与人工标注、“卡点自然”评估口径、基准硬件和效果/性能阈值；随后要求按 A-031 0.2 获取实际视频。D-010/A-031 0.3 形成后，用户再次明确继续 T-029，仅获取/probe 4 个替换来源、更新 manifest 和验证配额，满足后核对 H-013；暂时禁止越过真人裁决标注和 Windows 基准条件运行正式 classic 门禁。2026-09-15，用户在获知独立性和外推限制后确认 D-014，将本阶段主观评审改为个人单用户验收；同日又明确要求依据 A-031 0.5/D-014 继续准备 `USER-01` 三阶段流程，并在当前 `TIGER` 最佳性能模式下执行 T-028 Release PE 一次预热加五次正式测量；真实 VFR 和 overall 门禁不变。
- 目标与范围：在代表性数据上评估镜头、运动和动作候选的效果、可解释性、人工修正量与性能，决定是否具备进入可选模型评估的证据；不自行确定产品门槛或引入模型。
- 输入与依赖：T-027、T-028 已完成；D-009～D-011、D-014 confirmed；A-031 0.5 approved，已确认 40 条 B 站产品主集、独立真实 VFR 技术集双 gate 和个人单用户验收协议，H-017 已由发起人复核关闭；A-011 0.1、A-028 0.2、A-029 0.1、A-030 0.4。
...
- 进展：负责人按 D-010/A-031 0.3 获取并 probe `GAME-004`、`TRAVEL-001`、`TRAVEL-007`、`LIFE-007` 四个替换来源，保留其余 36 个代理，形成 dataset 0.2.0。40/40 媒体 SHA 复核一致，总计 1,359,437,599 bytes、1,815.044 秒；来源时间窗、`calibration/tuning/final=4/16/20` 及舞蹈/游戏/旅行/生活各 10 条均通过。实际结果同时否定 D-010 的 VFR 选源假设：完整数据为 CFR 40、VFR 0。2026-09-15 接收 A-031 0.5/D-014/H-017 后，负责人建立 `USER-01` 参考制作、隐藏随机化 A/B 盲评和人工修正三阶段工具/界面/冻结校验；重新核对 40/40 媒体和源逐帧 PTS，生成 40 个 Git 忽略的静音 H.264 预览，并冻结 preview ordinal→source true `timeNs` 映射。参考会话 40 条中 final 20 条，界面已可用，但实际人工记录仍为 0。当前 `TIGER` 的 AC Best Performance overlay 和 SAC=0 已同轮保存，T-028 Release PE 在 OpenCV 8/FFmpeg 2 线程下完成一组有效 1 warmup + 5 measured；wall 样本 `201162/186496/175500/174688/176782 us`、取消 P95 `2557 us`，原生视频分析单测 8/8 通过。该 synthetic 160×90 基线仅证明主机/PE 就绪，不构成正式 T-029 P-*；依顺序门禁仍未运行产品/VFR 正式 classic，未提出模型方案、未引入 ONNX。
...
- 阻塞与下一位行动人：H-013 关闭条件未满足，保持 `accepted`；仍须取得至少 3 条且 final 至少 1 条、至少 2 个设备/软件族的真实 VFR 原始媒体，B 站平台转码流不能替代。H-014 的 Windows Release 子条件已测量，但实际单用户证据仍缺；下一位行动人是 `USER-01`，在已提供的本地界面完成 40 条产品主集人工参考并导出原始 JSON。负责人冻结 `single_user_reference` 后才可运行对应 classic、创建隐藏 A/B 盲评；每条盲评提交后才可开放修正。真实 VFR、实际人工证据及正式产品/VFR P-* 未齐前，overall 必须保持 `not-evaluated`。
- 更新日期：2026-09-15。
...
- 负责人：video-algorithm-engineer-cv-01
- 状态：completed
- 授权来源与日期：本会话用户于 2026-09-09 要求补充系统架构师判断的新增成员及成员任务并提交；同一用户于 2026-09-14 明确要求初始化成员身份、接收 H-007 并执行 T-027，完成后才进入 T-028/T-029。
- 目标与范围：定义代理帧输入、视频候选、参数/算法版本、失败诊断、代表性样本和效果/性能测量方法；不设定未经产品确认的门槛，不重定义媒体或核心时间。
- 输入与依赖：A-002 0.2、A-004 0.5、A-005 0.4、A-006 0.1 WP-05、A-011 0.1、A-012 0.1、A-014 0.4、A-015 0.3、A-016 0.1；D-001～D-008 confirmed；T-013/T-014/T-017/T-018/T-020 已完成，可立即开展。
- 优先级：未设定（架构建议：立即启动）。
...
- 阻塞与下一位行动人：architect-01 已独立复核提交 `6e3f37d`，确认 `T026-DEFECT-005`、`006` 修复及针对性回归通过，T-026 无范围内实现阻塞，H-006 已关闭。视觉风格、产品文案、产品默认音色、发布导出格式、产品默认视觉模板参数、设备矩阵及可访问性/性能门槛继续待确认；A-018 CC0 映射仅能显式作为开发用途，当前导出仍为 T-019 `testOnly`。T-022 仍等待 T-029；下一条项目执行任务为 video-algorithm-engineer-cv-01 接收 H-007 并执行 T-027。
- 更新日期：2026-09-12。
