...
| A-002 | [书稿打磨计划](A-002-book-polishing-plan.md) | project-manager | T-002 | 0.3 | draft | 范围依据 D-001~D-007（confirmed）；计划文本待确认 |
| A-003 | [知识点校验清单](A-003-知识点校验清单.md) | writer | T-003 | 0.1 | draft | 尚无 |
| A-004 | [体例约定](A-004-体例约定草案.md) | writer | T-005 | 0.3 | draft（v0.3 为 §7 事实修正与 §4 三条补充条款，待确认；v0.2 内容不变） | v0.2：2026-09-19 用户确认（章标题层级按方案 A；章节 git tag 事宜用户自行决定、不纳入约定）；v0.3 待确认 |
