...
- 工程位置：迁移前产品工程位于 `opc` 仓库根；按 [D-016](DECISIONS.md)，迁移后的唯一产品工程根为 `projects/space-rhythm/workspace/`，包含 CMake/preset/vcpkg 入口以及 `cmake/`、`src/`、`tests/`、`tooling/` 和工程 `docs/`。仓库级 `.github/workflows/` 留在根目录但从该工程根执行；项目管理资料位于其上级 `projects/space-rhythm/`，框架规则、岗位、知识、模板和校验脚本不属于产品工程，不随迁移移动。
