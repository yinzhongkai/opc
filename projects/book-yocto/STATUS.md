...
- 汇总日期与信息截至点：2026-09-27；信息范围为 T-011 稳定版 Git `5babfb7`、project-manager 既有复核结论及用户追加的 U-17（chapter 2 所有图片统一改为 ASCII，并授权修改后直接安排对应成员 review）。
- 维护人：project-manager（按 [PROJECT.md](PROJECT.md) 指定的协调记录维护人）。
- 当前阶段：P1 基础修复已完成；**P2 章节闭环试点已完成（chapter 1 于 2026-09-25 定稿）**；**P3 已启动，chapter 2 在 Git `5babfb7` 基础上收到 U-17：Fig-2-1~Fig-2-4 全部统一为 ASCII。任务已退回 writer；完成后由 project-manager 呈现复核，并按用户授权直接交 reviewer 做四图技术映射及 U-11/U-13/U-14/U-15 的针对性复核，之后交用户确认 chapter 2 定稿与 A-004 v0.5**（计划见 A-002 v0.5）。目标与范围见 [PROJECT.md](PROJECT.md) 与 D-001~D-011。书稿位于 `projects/book-yocto/workspace/yocto/`。
- 任务进展：
  ...
  - T-011 chapter 2《读懂这个项目》修订与第 1 轮评审（writer/reviewer/用户）：in_progress——Git `5babfb7` 已处理 U-9~U-16，但用户于 2026-09-27 追加 U-17，要求 Fig-2-1~Fig-2-4 全部统一为 ASCII；当前只有 Fig-2-1 符合，Fig-2-2~Fig-2-4 已退回 writer 改图并删除对应 SVG。
...
- 阻塞、待确认事项及下一位行动人：
  - **下一位行动人 writer**：按 U-17 将 Fig-2-2~Fig-2-4 改为 ASCII、删除三份 SVG、提交新的稳定版本和处理回复。随后 project-manager 复核呈现，并直接通知 reviewer 复核四图技术映射及 U-11/U-13/U-14/U-15；提交结论后转用户确认。
