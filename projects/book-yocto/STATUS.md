...
- 汇总日期与信息截至点：2026-09-27；信息范围为 T-011 定稿前统一修订最终稳定版 Git `5babfb7`、writer 处理回复及 project-manager 对 U-9/U-10/U-12/U-16 与 T-013 的复核结论。
- 维护人：project-manager（按 [PROJECT.md](PROJECT.md) 指定的协调记录维护人）。
- 当前阶段：P1 基础修复已完成；**P2 章节闭环试点已完成（chapter 1 于 2026-09-25 定稿）**；**P3 已启动，chapter 2 的 U-9~U-16 与 T-013 已由 writer 修订为 Git `5babfb7`，project-manager 对呈现、排版、规则与意见闭环复核通过；等待 reviewer 对 U-11/U-13/U-14/U-15 做针对性技术复核，之后交用户确认 chapter 2 定稿与 A-004 v0.5**（计划见 A-002 v0.5）。目标与范围见 [PROJECT.md](PROJECT.md) 与 D-001~D-011。书稿位于 `projects/book-yocto/workspace/yocto/`。
- 任务进展：
  ...
  - T-011 chapter 2《读懂这个项目》修订与第 1 轮评审（writer/reviewer/用户）：in_review——定稿前统一修订最终稳定版 Git `5babfb7` 已处理 U-9~U-16；project-manager 于 2026-09-27 复核 U-9/U-10/U-12/U-16、Fig-2-1 单行启动链呈现及意见闭环通过。等待 reviewer 复核 U-11 的固定启动链、U-13 的 feature 关系、U-14 的选择规则及 U-15 的 systemd/udev 失败链。
...
- 阻塞、待确认事项及下一位行动人：
  - **下一位行动人 reviewer**：针对 Git `5babfb7` 复核 U-11 的启动链（含 Fig-2-1/2/3 与 Table-2-2）、U-13 的 feature 关系、U-14 的 layer/recipe/version/provider 选择边界及 U-15 的 systemd/usrmerge/udev provider 失败链；提交结论后转用户确认。
  - **用户后续事项**：待 reviewer 针对性复核通过后确认 chapter 2 定稿及 A-004 v0.5；继续全书通读校验（A-003），chapter 8 时顺带裁决 X-1；另需安排四个开发态仓库工程创建与构建环境的责任人（P0.5 前置，chapter 6 以后修订依赖）。
