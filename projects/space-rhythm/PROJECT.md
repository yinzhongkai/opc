...
- 当前阶段：一期 MVP 个人试用工程、工程根迁移及迁移后未签名 Windows 交付复验均已完成。依据 D-016，T-040 已将完整产品工程迁入 `projects/space-rhythm/workspace/`，T-041 已完成三套 preset 各 166/166 的独立工程回归与路径隔离验证，T-042 已从新工程根完成确定性个人未签名包重建和当前 `TIGER` 完整交付链复验。用户可继续个人试用，后续使用中发现的问题另立缺陷或任务；T-022/T-038 证据和冻结包保留为迁移前基线。
...
- 工程位置：迁移前产品工程位于 `opc` 仓库根；按 [D-016](DECISIONS.md)，迁移后的唯一产品工程根为 `projects/space-rhythm/workspace/`，包含 CMake/preset/vcpkg 入口以及 `cmake/`、`src/`、`tests/`、`tooling/`、工程 `docs/` 和 `.github/workflows/`。仓库根不保留 GitHub 工作流入口，因此迁移后 GitHub Actions 暂时停用；项目管理资料位于其上级 `projects/space-rhythm/`，框架规则、岗位、知识、模板和校验脚本不属于产品工程，不随迁移移动。
