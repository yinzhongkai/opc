...
- 当前阶段：一期 MVP 集成前收口与产品效果评估；T-029 因产品评估输入缺失处于 blocked，Windows 发布计划 T-036 尚待启动。
...
- 交付物与验收要求：当前成果均以 `artifacts/` 索引和任务证据为准；产品效果须完成 T-029，端到端故障恢复及 G0～G4 证据须完成 T-022，Windows 发布候选须完成 T-036～T-038。任务完成不自动代表成果批准或生产发布批准。
- 时间、资源与其他约束：尚未确认工期、预算、代表产品素材、自然度与性能阈值、最低 Windows 版本、正式导出格式、安装器、签名主体/证书和发布渠道；用户保证当前仓库各成员会话串行执行。
